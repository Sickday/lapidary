...
on_magic_on_item(1162){|player, id, slot|
  ...
  if (fire_staff || player.inventory.count(554) >= 3) && player.inventory.contains(561)
    ...
    item = Lapidary::Item::Item.new(id, 1)
    
    player.inventory.remove(-1, Lapidary::Item::Item.new(554, 3)) unless fire_staff
    player.inventory.remove(-1, Lapidary::Item::Item.new(561, 1))
    player.inventory.remove(slot, item)
    player.inventory.add(Lapidary::Item::Item.new(995, item.definition.lowalc))
    player.skills.add_exp(:magic, 31)
    player.play_animation Lapidary::World::Animation.new(712)
    player.play_graphic Lapidary::World::Graphic.new(112, 2)
  else
...
on_magic_on_item(1178){|player, id, slot|
  ...
  if (fire_staff || player.inventory.count(554) >= 5) && player.inventory.contains(561)
    ...
    player.inventory.remove(-1, Lapidary::Item::Item.new(554, 5)) unless fire_staff
    player.inventory.remove(-1, Lapidary::Item::Item.new(561, 1))
    player.inventory.remove(slot, item)
    player.inventory.add(Lapidary::Item::Item.new(995, item.definition.highalc))
    player.skills.add_exp(:magic, 65)
    player.play_animation Lapidary::World::Animation.new(713)
    player.play_graphic Lapidary::World::Graphic.new(113, 2)
  else
    player.io.send_message "You do not have the required runes to cast this spell."
  end
}
