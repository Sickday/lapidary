...
on_command("objspawn") {|player, params|
 ...
 object = Lapidary::Objects::Object.new(params[0].to_i, temp_loc, 2, params[1].to_i, -1, temp_loc, 0, params[2].to_i)
 object.change 
...
on_command("update") {|player, params|
  ...
  WORLD.submit_event Lapidary::Tasks::SystemUpdateEvent.new(time)
}
...
on_command("item") {|player, params|
  ...
  player.inventory.add Lapidary::Item::Item.new(id, count)
}
...
on_command("spawn") {|player, params|
   id = params[0].to_i
   npc = Lapidary::NPC::NPC.new Lapidary::NPC::NPCDefinition.for_id(id)
...
on_command("tele") {|player, params|
  ...
  loc = Lapidary::Model::Location.new(x, y, z)
...
on_command("g") {|player, params|
  ...
  player.face Lapidary::Model::Location.new(x, y, z)
}

on_command("max") {|player, params|
  Lapidary::Player::Skills::SKILLS.each {|skill|
...

{
  ...
  on_int_button(button) {|player|
    player.play_animation Lapidary::Model::Animation.new(anim)
  }
}
