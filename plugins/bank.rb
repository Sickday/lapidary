...
on_command("bank") {|player, params|
  r = Random.new
  (0..20).each {|e|
    player.bank.add Lapidary::Item::Item.new(r.rand(1040...1056))
  }
  Bank.open player
}
...
class Bank
  def self.open(player)
    ...
    player.interface_state.add_listener player.bank, Lapidary::Item::InterfaceContainerListener.new(player, 5382)
    player.interface_state.add_listener player.inventory, Lapidary::Item::InterfaceContainerListener.new(player, 5064)
  end
  ...
  def self.withdraw(player, slot, id, amount)
    ...
    definition = Lapidary::Item::ItemDefinition.for_id new_id
    ...
    if player.inventory.add Lapidary::Item::Item.new(new_id, transfer_amount)
      # All items in the bank are stacked, makes it very easy!
      new_amount = item.count - transfer_amount
      if new_amount <= 0
        player.bank.set slot, nil
      else
        player.bank.set slot, Lapidary::Item::Item.new(item.id, new_amount)
      end
    else
      player.io.send_message "You don't have enough inventory space to withdraw that many."
    end
  ...
  def self.deposit(player, slot, id, amount)
    ...
    begin 
      ...
      if (item.definition.stackable || false) || (item.definition.noted || false)
        ...
        new_item = nil
        if new_inv_amount > 0
          new_item = Lapidary::Item::Item.new item.id, new_inv_amount
        end
        if !player.bank.add Lapidary::Item::Item.new(banked_id, transfer_amount)
          player.io.send_message "You don't have enough space in your bank account."
        else
        ...
        if !player.bank.add Lapidary::Item::Item.new(item.id, transfer_amount)
          player.io.send_message "You don't have enough space in your bank account."
        else
