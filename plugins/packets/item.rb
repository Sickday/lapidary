...
on_packet(87) {|player, packet|
  ...
  if item.id == id
    gitem = Lapidary::GroundItems::GroundItem.new player, item
...
on_packet(236) {|player, packet|
  ...
  loc = Lapidary::Model::Location.new item_x, item_y, player.location.z
  item = nil
  
  # Check if this item is a world item
  world_item = false
  Lapidary::World::ItemSpawns.items.each {|i|
  ...
  unless item == nil
    ...
    player.play_animation Lapidary::Model::Animation.new(832, 10) if item.on_table
    player.action_queue.add(Lapidary::GroundItems::PickupItemAction.new(player, item))
  end
}
...
on_packet(192) {|player, packet|
  ...
  item_id = packet.read_short.ushort
  loc = Lapidary::Model::Location.new obj_x, obj_y, player.location.z
