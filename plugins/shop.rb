...
on_item_option(3900) {|player, id, slot|
  value = Lapidary::Shops::ShopManager.buy_value(player, slot).to_i
  name = Lapidary::Item::ItemDefinition.for_id(id).name
...
on_item_option(3823) {|player, id, slot|
  unless player.current_shop == nil
    value = Lapidary::Shops::ShopManager.sell_value(player, slot).to_i
    name = Lapidary::Item::ItemDefinition.for_id(id).name
...
on_item_option2(3823) {|player, id, slot|
  Lapidary::Shops::ShopManager.sell player, slot, id, 1
}

on_item_option2(3900) {|player, id, slot|
  Lapidary::Shops::ShopManager.buy player, slot, id, 1
}

# Item buy/sell amount 5
on_item_option3(3823) {|player, id, slot|
  Lapidary::Shops::ShopManager.sell player, slot, id, 5
}

on_item_option3(3900) {|player, id, slot|
  Lapidary::Shops::ShopManager.buy player, slot, id, 5
}

# Item buy/sell amount 10
on_item_option4(3823) {|player, id, slot|
  Lapidary::Shops::ShopManager.sell player, slot, id, 10
}

on_item_option4(3900) {|player, id, slot|
  Lapidary::Shops::ShopManager.buy player, slot, id, 10
}
...
on_command("shop") {|player, params|
  Lapidary::Shops::ShopManager.open(1, player)
}