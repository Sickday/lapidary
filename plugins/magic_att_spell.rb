...
def projectile(src, dest, angle, speed, id, start_z, end_z, index)
  offset = Lapidary::Model::Location.new(-(src.x-dest.x), -(src.x-dest.y), 0)
  
  WORLD.region_manager.get_local_players(src).each {|p|
    if p && p.location.within_distance?(src)
      # Region
      region = Lapidary::Net::PacketBuilder.new(85)
      ...
      # Graphic
      graphic = Lapidary::Net::PacketBuilder.new(117)
...
def stillgfx(id, x, y, z)
  ...
  WORLD.region_manager.get_local_players(loc).each {|p|
    if p && p.location.within_distance?(loc)
      ...
      # Graphic
      graphic = Lapidary::Net::PacketBuilder.new(4)
...
on_magic_on_npc(1183) {|player, npc|
  ...
  player.play_graphic Lapidary::Model::Graphic.new(158, 6553600)
  player.play_animation Lapidary::Model::Animation.new(711)
