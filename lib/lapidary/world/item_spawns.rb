module Lapidary::World
  ...
  class Item
    ...
    def initialize(data)
      @item = Lapidary::Item::Item.new(data['id'].to_i, (data.include?('amount') ? data['amount'].to_i : 1))
      @location = Lapidary::Model::Location.new(data['x'].to_i, data['y'].to_i, data['z'].to_i)
