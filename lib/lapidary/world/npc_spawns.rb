module Lapidary::World
  class NPCSpawns
    ...
    def NPCSpawns.spawn(data)
      npc = Lapidary::NPC::NPC.new Lapidary::NPC::NPCDefinition.for_id(data['id'].to_i)
      npc.location = Lapidary::Model::Location.new(data['x'].to_i, data['y'].to_i, data['z'].to_i)
      ...
      if data.include?('shop')
        ...
        if !handler.instance_of?(Proc)
          on_npc_option2(data['id'].to_i) {|player, npc|
            Lapidary::Shops::ShopManager.open(data['shop'].to_i, player)
