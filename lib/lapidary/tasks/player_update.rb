module Lapidary::Tasks
  ...
  class PlayerUpdateTask
    ...
    def execute
      ...
      update_block = Lapidary::Net::PacketBuilder.new
      packet = Lapidary::Net::PacketBuilder.new(81, :VARSH)
    ...
    def append_chat(p, block)
      ...
      block.add_byte Lapidary::World::RIGHTS.index(p.rights)
    ...
    def append_appearance(p, block)
      ...
      props = Lapidary::Net::PacketBuilder.new
      ...
      if p.model != -1
        ...
        if eq.is_slot_used(4) && Lapidary::Equipment.is(eq.items[4], 10)
          props.add_short (0x200 + eq.items[4].id).short
        else
        ...
        if eq.is_slot_used(0) && (Lapidary::Equipment.is(eq.items[0], 11) || Lapidary::Equipment.is(eq.items[0], 12))
          props.add_byte 0
        else
        ...
        # Beard
        fullhelm = eq.is_slot_used(0) && (Lapidary::Equipment.is(eq.items[0], 11) || Lapidary::Equipment.is(eq.items[0], 12))
      ...
      # Details
      props.add_long Lapidary::Misc::NameUtils.name_to_long(p.name)
    ...
    def update_player(packet, p, force_appearance, no_chat)
      ...
      block = Lapidary::Net::PacketBuilder.new
      flags = p.flags
      mask = 0
