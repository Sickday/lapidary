module Lapidary::Doors
    ...
    def load_single_doors
      ...
      d["door"].each_with_index {|row, idx|
        @@single_data << 
          {
            :id => row['id'].to_i,
            :location => Lapidary::Model::Location.new(row['x'].to_i, row['y'].to_i, row['z'].to_i),
            :face => row['face'].to_i,
            :type => row['type'].to_i
          }
      }
  ...
  class Door < Lapidary::Objects::Object
    attr :open
  ...
  class DoubleDoor < Door
    ...
    def initialize(data, open = true) 
      ...
      temp_l = Lapidary::Model::Location.new(data[:location].x+l_x_off, data[:location].y+l_y_off, data[:location].z)
      temp_r = Lapidary::Model::Location.new(data[:location].x+r_x_off, data[:location].y+r_y_off, data[:location].z)
      ...
      if l_data != nil
        ...
        @orig_id = @id
        @orig_location = Lapidary::Model::Location.new(@location.x, @location.y, @location.z)
        @orig_face = @face
        ...
        @r_door_orig_location = Lapidary::Model::Location.new(@r_door_location.x, @r_door_location.y, @r_door_location.z)
